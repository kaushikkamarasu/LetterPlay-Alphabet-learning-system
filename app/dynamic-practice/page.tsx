...
export default function PracticePage() {
  ...
  const [letterMistakes, setLetterMistakes] = useState(
    devanagariLetters.map(letter => ({ letter, mistakes: 0 }))
  )
  const [tempMistakes, setTempMistakes] = useState([...letterMistakes])
  

//   const currentLetter = devanagariLetters[currentLetterIndex]
    const currentLetter = letterMistakes[currentLetterIndex]?.letter || devanagariLetters[0]
...
//   const handleFinish = () => {
//     // Update the original mistake state
//     setLetterMistakes([...tempMistakes].sort((a, b) => b.mistakes - a.mistakes))
  
//     // Reset current letter index to start from the most mistaken letter
//     setCurrentLetterIndex(0)
//     console.log("LOOK HERE");
//     console.log("LOOK HERE");
//     console.log(letterMistakes);
...
//   }
    const handleFinish = () => {
        ...
        const updatedMistakes = [...tempMistakes].sort((a, b) => b.mistakes - a.mistakes);
        console.log("Updated letterMistakes inside setState:", updatedMistakes);
        return updatedMistakes;
        });
    
        setCurrentLetterIndex((prev) => {
            console.log("Updated Letter Index");
            return 0;
        });
...
//     //   const predictedLetter = response.data.prediction
//       const predictedLetter = response.data.prediction;
//       setPrediction(predictedLetter)
  
//       // If incorrect, increment mistakes in tempMistakes
...
//         setTempMistakes(prev => {
//             const updatedMistakes = prev.map(entry =>
//               entry.letter === currentLetter ? { ...entry, mistakes: entry.mistakes + 1 } : entry
//             );
            
//             console.log("Updated tempMistakes inside setState:", updatedMistakes);
//             return updatedMistakes;
//           });
...
const handleCheckLetter = async () => {
  ...
  try {
    ...
    tempCanvas.toBlob(async (blob) => {
      ...
      if(predictedLetter === currentLetter) {
        // const bruh = entry.mistakes - 1 < 0 ? 0 : entry.mistakes - 1;
        setTempMistakes(prev => {
          const updatedMistakes = prev.map(entry =>
            entry.letter === currentLetter ? { ...entry, mistakes: entry.mistakes - 1 < 0 ? 0 : entry.mistakes -1  } : entry
          );
          console.log("Updated tempMistakes inside setState:", updatedMistakes);
          return updatedMistakes;
        });
...
{/* Finish Button */}
            ...
            <Button
                ...
                className="rounded-full bg-blue-500 hover:bg-blue-600 text-white px-8 py-3 text-lg"
            >
                Finish
            </Button>
            </div>
